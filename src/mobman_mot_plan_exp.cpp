...
double stepSizeTrans(double t, double tf, double ts, double blendPerc);
template <class MatT>
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    while (trajDuration < tf)
    {
        ...
        trans.push_back(stepSizeTrans(trajDuration, tf, ts, 0.2));
    ...
    std::cout << "Desired final pos: " << endl
              << desiredTraj.getPose(trajDuration) << endl;
...
double stepSizeTrans(double t, double tf, double ts, double blendPerc)
{
    ...
    if (t < tb1)
    {
        stepSize = a1 * pow(t, 3) + a2 * pow(t, 4) + a3 * pow(t, 5);
    }
    if (t >= tb1 && t <= tb2)
    {
        stepSize = 1.0;
    ...
    {
        deltaTime = t - tb2;
        stepSize = 1.0 - a1 * pow(deltaTime, 3) - a2 * pow(deltaTime, 4) - a3 * pow(deltaTime, 5);
    }
    return stepSize;
}
